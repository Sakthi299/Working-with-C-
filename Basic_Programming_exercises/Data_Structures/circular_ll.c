...
printf("\n 8: Add a node after a given node");
printf("\n\n Enter your option : "); scanf("%d", &option); 
switch(option) 
{  case 1: start = create_cll(start); break; 
...
case 8: start = insert_after(start);  break;  
}  
}while(option < 9);
return 0; 
} 
...
printf("\t %d", ptr -> data);//To display last node
return start; 
} 
...
printf("\n Enter the data : "); 
scanf("%d", &num); 
printf("Enter the val after which node is to be inserted:");
scanf("%d", &val);
new_node = (struct node *)malloc(sizeof(struct node)); 
new_node -> data = num; 
ptr = start;
...
ptr -> next = start -> next; 
free(start); 
start = ptr -> next;//fixing 2nd node as START
return start; } 
